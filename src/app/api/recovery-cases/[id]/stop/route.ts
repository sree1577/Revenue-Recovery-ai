...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!parsed.success) return Response.json({ success: false, data: null, error: { code: "REASON_REQUIRED", message: "A stop reason is required." } }, { status: 400 });
  try {
    const result = await prisma.$transaction(async (database) => {
      ...
      await database.auditLog.create({ data: { recoveryCaseId: recoveryCase.id, eventType: "CASE_STOPPED", actor: AuditActor.MERCHANT, decision: parsed.data.reason, outputSnapshot: { status: RecoveryCaseStatus.STOPPED }, policyVersion: "2026-09-01" } });
      return "ok" as const;
    });
    ...
    if (result === "invalid") return Response.json({ success: false, data: null, error: { code: "CASE_NOT_STOPPABLE", message: "This case is already terminal and cannot be stopped." } }, { status: 409 });
    return Response.json({ success: true, data: { status: RecoveryCaseStatus.STOPPED }, error: null });
  } catch {
    return Response.json({ success: false, data: null, error: { code: "STOP_FAILED", message: "Stopping recovery could not be saved." } }, { status: 500 });
  }
}
